...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            subprocess.check_call('%s && drive upload -f %s/backup/backup_mysql.gz -t Primerize_%s_mysql%s.gz' % (gdrive_dir, MEDIA_ROOT, d, prefix), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        ...
        try:
            subprocess.check_call('%s && drive upload -f %s/backup/backup_static.tgz -t Primerize_%s_static%s.tgz' % (gdrive_dir, MEDIA_ROOT, d, prefix), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError:
        ...
        try:
            subprocess.check_call('%s && drive upload -f %s/backup/backup_apache.tgz -t Primerize_%s_apache%s.tgz' % (gdrive_dir, MEDIA_ROOT, d, prefix), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError:
        ...
        try:
            subprocess.check_call('%s && drive upload -f %s/backup/backup_config.tgz -t Primerize_%s_config%s.tgz' % (gdrive_dir, MEDIA_ROOT, d, prefix), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError:
        ...
        try:
            ...
            list_mysql = subprocess.Popen("%s && drive list -q \"title contains 'Primerize_' and (title contains '_mysql.gz' or title contains '_mysql_DEBUG.gz') and modifiedDate <= '%s'\"| awk '{printf $1\" \"}'" % (gdrive_dir, old), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0].strip().split()[1:]
            list_static = subprocess.Popen("%s && drive list -q \"title contains 'Primerize_' and (title contains '_static.tgz' or title contains '_static_DEBUG.tgz') and modifiedDate <= '%s'\"| awk '{ printf $1\" \"}'" % (gdrive_dir, old), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0].strip().split()[1:]
            list_apache = subprocess.Popen("%s && drive list -q \"title contains 'Primerize_' and (title contains '_apache.tgz' or title contains '_apache_DEBUG.tgz') and modifiedDate <= '%s'\"| awk '{ printf $1\" \"}'" % (gdrive_dir, old), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0].strip().split()[1:]
            list_config = subprocess.Popen("%s && drive list -q \"title contains 'Primerize_' and (title contains '_config.tgz' or title contains '_config_DEBUG.tgz') and modifiedDate <= '%s'\"| awk '{ printf $1\" \"}'" % (gdrive_dir, old), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).communicate()[0].strip().split()[1:]
