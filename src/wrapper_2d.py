...
def design_2d_run(request):
    ...
    if form.is_valid():
        ...
        job_entry = Design2D(date=datetime.now(), job_id=job_id, sequence=sequence, tag=tag, status='1', params=simplejson.dumps({'offset': offset, 'min_muts': min_muts, 'max_muts': max_muts, 'which_lib': which_lib}, sort_keys=True, indent=' ' * 4))
        job_entry.save()
...
def design_2d_wrapper(sequence, primer_set, tag, offset, which_muts, which_lib, job_id):
    ...
    try:
        ...
        script += '</pre></div></div></div></div><div class="row"><div class="col-lg-9 col-md-9 col-sm-9 col-xs-9"><p class="lead"><span class="glyphicon glyphicon-question-sign"></span>&nbsp;&nbsp;<b><u><i>What\'s next?</i></u></b> Try our suggested experimental <a id="btn-result-to-protocol" class="btn btn-info btn-sm btn-spa" href="/protocol/#par_prep" role="button" style="color: #ffffff;"><span class="glyphicon glyphicon-file"></span>&nbsp;&nbsp;Protocol&nbsp;</a> for PCR assembly. Or go ahead for <code>Mutation/Rescue Sets</code>.</p></div><div class="col-lg-3 col-md-3 col-sm-3 col-xs-3"><a id="btn-2d-to-3d" class="btn btn-primary btn-block btn-spa" href="/design_3d_from_2d/" role="button" style="color: #ffffff;"><span class="glyphicon glyphicon-play-circle"></span>&nbsp;&nbsp;Design 3D&nbsp;</a></div></div>'
        ...
        script = script.replace('__SEQ_ANNOT__', illustration_1 + '</p><p class="text-right" style="margin-top:2px;">&nbsp;<span class="monospace">' + illustration_2 + '</p><p class="text-right" style="margin-top:0px;">&nbsp;<span class="monospace">' + illustration_3)
