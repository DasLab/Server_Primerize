...
def create_wait_html(job_id, type):
    html = '<br/><hr/><div class="row"><div class="col-md-12"><h2><span class="glyphicon glyphicon-hourglass"></span>&nbsp;&nbsp;Primerize is running...  </h2></div></div><br/><div class="progress"><div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100" style="width: 100%%;"><span class="sr-only"></span></div></div><h3 class="text-center">Your <span class="label label-violet">JOB_ID</span> is: <span class="label label-inverse">%s</span></h3><br/><br/><img class="center-block" src="/site_media/images/fg_load.gif" width="48px" style="opacity:0.5;"/><br/><br/><div class="row"><div class="col-md-6"><p>Your query is being processed. Usually, the calculation is finished within 30 seconds. Depending on the input sequence length and complexity, the run may take longer.</p><p>You can close the browser and retrieve the result later using the above unique <span class="label label-violet">JOB_ID</span>. The cached result expires after 9 months.</p></div><div class="col-md-6"><p class="text-center well" ><b><span class="glyphicon glyphicon-copy"></span>&nbsp;&nbsp;Please <button id="btn-copy" class="btn btn-success" data-clipboard-text="http://primerize.stanford.edu/result/?job_id=%s">Copy</button> this link: <br/><code id="url_id">http://primerize.stanford.edu/result/?job_id=%s</code></b></p><br/></div><script type="text/javascript">var client = new ZeroClipboard( document.getElementById("btn-copy") );</script>' % (job_id, job_id, job_id)
    create_res_html(html, job_id, type)


def create_err_html(job_id, t_total, type):
    html = '<br/><hr/><div class="row"><div class="col-md-8"><h2><span class="glyphicon glyphicon-ban-circle"></span>&nbsp;&nbsp;Primerize has difficulty in your query...</h2></div><div class="col-md-4"><h4 class="text-right"><span class="glyphicon glyphicon-search"></span>&nbsp;&nbsp;<span class="label label-violet">JOB_ID</span>: <span class="label label-inverse">%s</span></h4><button href="%s" class="btn btn-info pull-right" style="color: #ffffff;" title="Output in plain text" download disabled>&nbsp;Save Result&nbsp;</button></div></div><br/><div class="progress"><div class="progress-bar progress-bar-danger progress-bar-striped" role="progressbar" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100" style="width: 100%%"><span class="sr-only"></span></div></div><br/><p>Primerize encountered an internal error while processing your query. Sorry for the inconvenience. </p><p>We will investigate and fix the problem.</p><p>For further information, please feel free to <a class="btn btn-warning btn-sm" href="/about/#contact" style="color: #ffffff;">Contact</a> us to track down the problem.</p>' % (job_id, '/site_data/1d/result_%s.txt' % job_id)
    if type == 1:
