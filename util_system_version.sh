...
echo "$(tput setab 39) jquery $(cut -d$'\t' -f10 cache/stat_sys.txt | sed 's/ $*//') | \
...
echo "$(tput setab 55) boto $(cut -d$'\t' -f24 cache/stat_sys.txt | sed 's/ $*//') | \
pygithub $(cut -d$'\t' -f25 cache/stat_sys.txt | sed 's/ $*//') | \
numpy $(cut -d$'\t' -f30 cache/stat_sys.txt | sed 's/ $*//') | \
...
openssl $(cut -d$'\t' -f9 cache/stat_sys.txt | sed 's/ $*//') | \
