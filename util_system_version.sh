...
numba $(cut -d$'\t' -f35 cache/stat_sys.txt | sed 's/ $*//') $(tput sgr 0)" >> ~/.ver_txt
...
curl $(cut -d$'\t' -f24 cache/stat_sys.txt | sed 's/ $*//') | \
gdrive $(cut -d$'\t' -f22 cache/stat_sys.txt | sed 's/ $*//') | \
yuicompressor $(cut -d$'\t' -f36 cache/stat_sys.txt | sed 's/ $*//') $(tput sgr 0)" >> ~/.ver_txt
...
requests $(cut -d$'\t' -f28 cache/stat_sys.txt | sed 's/ $*//') | \
simplejson $(cut -d$'\t' -f29 cache/stat_sys.txt | sed 's/ $*//') $(tput sgr 0)" >> ~/.ver_txt
...
echo "$(tput setab 55) numpy $(cut -d$'\t' -f32 cache/stat_sys.txt | sed 's/ $*//') | \
scipy $(cut -d$'\t' -f33 cache/stat_sys.txt | sed 's/ $*//') | \
matplotlib $(cut -d$'\t' -f34 cache/stat_sys.txt | sed 's/ $*//') | \
boto $(cut -d$'\t' -f25 cache/stat_sys.txt | sed 's/ $*//') | \
pygithub $(cut -d$'\t' -f26 cache/stat_sys.txt | sed 's/ $*//') | \
...
echo "$(tput setab 15)$(tput setaf 16) rdatkit $(cut -d$'\t' -f37 cache/stat_sys.txt | sed 's/ $*//') | \
na_thermo $(cut -d$'\t' -f38 cache/stat_sys.txt | sed 's/ $*//') $(tput sgr 0)" >> ~/.ver_txt

echo "$(tput setab 8) git $(cut -d$'\t' -f19 cache/stat_sys.txt | sed 's/ $*//') | \
pip $(cut -d$'\t' -f31 cache/stat_sys.txt | sed 's/ $*//') | \
