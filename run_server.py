...
class Root:
    ...
    def admin(self):
        ...
        (ver_linux, ver_python, ver_cherrypy, ver_matlab, ver_rdatkit, ver_jquery, ver_bootstrap, ver_ssh, ver_screen, ver_tty, ver_apache, ver_git, ver_gcc, ver_clang, ver_cmake, ver_numpy, ver_scipy, ver_matplotlib, ver_celery, ver_simplejson, ver_setuptools, ver_pip, ver_octave, disk_sp, cache_n, cache_sz, path_primerize, path_nathermo, path_rdatkit, path_python, path_matlab) = tuple(f.readlines()[0].split('\t'))
        f.close()
        ...
        script = script.replace("__DISK_SP__", disk_sp).replace("__CACHE_SZ__", cache_sz).replace("__CACHE_N__", cache_n).replace("__PRIMERIZE_PATH__", path_primerize).replace("__NATHERMO_PATH__", path_nathermo).replace("__RDATKIT_PATH__", path_rdatkit).replace("__PYTHON_PATH__", path_python).replace("__MATLAB_PATH__", path_matlab)
        return script
